...
echo "name,network,region,privateIpGoogleAccess,purpose,gatewayAddress" > $OUTPUTDIR/csv/$OUTPUT-$PROJECT.csv
count=0
while [ $count -lt ${#subnets[@]} ]; do
    gcloud compute networks subnets describe --format json --region ${regions[$count]} --project $PROJECT  ${subnets[$count]} | \
        ...
        jq -r -c '[.name,.network,.region,.privateIpGoogleAccess,.purpose,.gatewayAddress] | @csv' | \
